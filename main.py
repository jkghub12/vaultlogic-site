    ...
    if not BYPASS_BALANCE_CHECK:
        try:
            ...
            if balance < data.amount:
                add_log(f"CRITICAL REJECTION: Wallet holds ${balance:,.2f}. Required: ${data.amount:,.2f}")
                return {"status": "error", "message": "Insufficient Balance"}
        except Exception:
            return {"status": "error", "message": "Chain Sync Failed"}
    else:
        add_log(f"BYPASS ACTIVE: Initializing for {data.address[:8]}... without balance check.")
...
async def reset(address: str):
    if address in kernel.active_deployments:
        ...
        add_log(f"SESSION TERMINATED: User {address[:8]} disconnected.")
    return {"status": "ok"}
    ...
    <main id="dash" class="max-w-7xl mx-auto opacity-20 pointer-events-none transition-all duration-700">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
            <div class="stat-card p-8 border-l-2 border-cyan-500">
                <p class="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-4">Capital Under Management</p>
                <h2 id="pDisp" class="text-5xl font-black italic tracking-tighter">$0</h2>
            </div>
            <div class="stat-card p-8 border-l-2 border-emerald-500">
                <p class="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-4">Net Yield (80%)</p>
                <h2 id="yieldDisp" class="text-5xl font-black text-emerald-400 italic tracking-tighter">$0.0000</h2>
            </div>
            ...
            <div class="lg:col-span-4 stat-card p-10">
                ...
                <div class="space-y-10">
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="h-1 bg-gray-900 rounded-full overflow-hidden"><div id="lBar" class="h-full bg-cyan-500 transition-all duration-1000" style="width:100%"></div></div>
                        <div class="flex justify-between text-[10px] font-bold uppercase text-gray-600">
                            <span>Liquidity</span>
                            <span id="pPerc" class="text-purple-400">0%</span>
                        </div>
                        <div class="h-1 bg-gray-900 rounded-full overflow-hidden"><div id="pBar" class="h-full bg-purple-500 transition-all duration-1000" style="width:0%"></div></div>
                    </div>

                    <button id="execBtn" onclick="initKernel()" class="w-full py-5 bg-cyan-800 text-white font-black rounded-xl text-[11px] uppercase tracking-widest hover:bg-cyan-700">Initialize Kernel</button>
                </div>
            </div>
    ...
    <script>
        ...
        async function initKernel() {
            ...
            if (data.status === "success") {
                ...
                btn.innerText = "REJECTED: BELOW FLOOR";
                btn.classList.add('bg-red-700');
                setTimeout(() => { btn.innerText = "Initialize Kernel"; btn.classList.remove('bg-red-700'); }, 3000);
            }
        }
