...
# --- 1. MOUNT STATIC FILES (FOR THE LOGO) ---
# This allows the app to serve VLlogo.png from your main directory
app.mount("/static", StaticFiles(directory="."), name="static")

# --- 2. AGENTKIT IDENTITY SETUP ---
def get_llc_account():
    # Looks for your vaultlogic_wallet.json file
...
<body class="text-slate-200 min-h-screen flex flex-col items-center justify-center p-6">
    ...
    <div class="max-w-md w-full glass rounded-3xl p-8 shadow-2xl">
        ...
        <div class="mb-6">
            ...
            <p class="text-[11px] font-mono text-blue-400 bg-blue-500/10 p-2 rounded-lg break-all">
                """ + wallet_provider.get_address() + """
            </p>
        </div>

        <div class="bg-slate-900/50 p-6 rounded-2xl border border-slate-800 mb-6 text-center">
            <p class="text-xs text-slate-500 mb-1 uppercase tracking-tighter">Current Aave Yield</p>
        ...
        <div class="text-[10px] text-center text-slate-700">
            Last Scan: <span id="timestamp">--</span>
        </div>
    </div>
    ...
    <script>
        async function fetchVaultData() {
            try {
                ...
                document.getElementById('eth-bal').innerText = data.wallet.eth;
                document.getElementById('usdc-bal').innerText = data.wallet.usdc;
...
@app.get("/api/yield")
async def yield_api():
    # Pulls data from the yieldscout logic we built earlier
