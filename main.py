...
def add_log(msg):
    ...
    if len(system_logs) > 10: system_logs.pop(0)
...
async def connect(data: WalletConnect):
    try:
        ...
        add_log(f"Wallet Authorized: {data.address[:6]}...{data.address[-4:]}")
    ...
    <html>
        <head>
            ...
            <style>
                ...
                .btn {{ background:#00ffcc; color:#000; border:none; padding:15px 30px; font-weight:bold; cursor:pointer; letter-spacing:2px; }}
                #console {{ 
                ...
                .log-entry.active {{ color:#00ffcc; }}
            </style>
        </head>
        <body>
            <h1 style="letter-spacing:15px;">VAULTLOGIC</h1>
            <p style="color:#00ffcc; font-size:11px; margin-bottom:30px;">{vault_cache['status']}</p>
            <button id="cta" class="btn">INITIALIZE ENGINE</button>
            
            <div class="container" style="margin-top:40px;">{yield_cards}</div>

            <div id="console">
                <div style="color:#333; margin-bottom:10px; text-transform:uppercase; font-size:10px;">System Event Log</div>
            ...
            <script type="module">
                ...
                const config = defaultWagmiConfig({{ chains: [mainnet, base], projectId: '{WC_PROJECT_ID}', metadata: {{ name: 'VaultLogic' }} }});
                const modal = createWeb3Modal({{ wagmiConfig: config, projectId: '{WC_PROJECT_ID}', chains: [mainnet, base], themeMode: 'dark' }});
                ...
                watchAccount(config, {{
                    onChange(acc) {{
                        if (acc.isConnected) {{
                            ...
                            document.getElementById('cta').style.background = "#222";
