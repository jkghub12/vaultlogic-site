    ...
    <html>
        ...
        <body>
            <div class="container">
                ...
                <p>Phase Alpha: Active Liquidity Management (ALM).</p>
                <h2>II. Validation Tier</h2>
                <p>Stress-testing performed at the $500 entry level.</p>
                <h2>III. Regulatory Shield</h2>
                <p>Compliant with the Clarity Act 2026.</p>
            </div>
        </body>
    </html>
    """
...
async def get_vault(request: Request):
    ...
    disconnect_link = '<a href="#" onclick="window.disconnectWallet()" style="color:#ff4444; font-size:9px; text-transform:uppercase; text-decoration:none; letter-spacing:1px; margin-top:10px; display:block; cursor:pointer;">[ TERMINATE SESSION ]</a>' if vault_cache["is_connected"] else ""
        ...
        <body>
            <div class="mission-brief">
                ...
                <div style="background: #050505; border: 1px solid #222; padding: 15px; margin: 20px auto; max-width: 400px; font-family: monospace; font-size: 12px; text-align: left; border-left: 3px solid #00ffcc; line-height: 1.6;">
                    ...
                    <div style="color: #666;">> ASSET_USDC: <span style="color: #eee;">{vault_cache.get('usdc_balance', '1.50 USDC')}</span></div>
                </div>
                ...
                watchAccount(wagmiConfig, {{
                  onChange(account) {{
                    if (account.isConnected && account.address) {{
                      ...
                      }}).then(r => {{ if(r.ok) setTimeout(() => window.location.reload(), 800); }});
                    }}
                  }}
                }})
            </script>
